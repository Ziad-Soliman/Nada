    ...
    return (
      <div className="max-w-md w-full bg-slate-900/80 backdrop-blur-xl p-8 rounded-3xl shadow-[0_0_40px_rgba(0,0,0,0.5)] border border-white/10 text-center animate-fade-in relative overflow-hidden">
        ...
        <p className="text-slate-300 mb-8 text-md leading-relaxed">
          Welcome to Majesty Maths! First, enter your name to create your student profile.
        </p>
        ...
        <form onSubmit={handleNameSubmit} className="space-y-6 relative z-10">
          ...
          <Button type="submit" disabled={!name.trim()} className="w-full group">
            Create Profile <ArrowRight className="w-5 h-5 ml-2 group-hover:translate-x-1 transition-transform" />
          </Button>
        </form>
      </div>
    );
  ...
  return (
    <div className="max-w-3xl w-full bg-slate-900/90 backdrop-blur-xl p-8 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.6)] border border-white/10 animate-fade-in flex flex-col md:flex-row gap-10 items-center relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center relative">
        <h3 className="text-2xl font-bold text-white mb-2 font-['Orbitron'] tracking-wide">Student Profile</h3>
        <h4 className="text-lg text-cyan-400 mb-6 font-medium">{name}</h4>
        ...
        <div className="bg-slate-950/50 px-4 py-2 rounded-lg border border-white/5 mt-2 text-center">
           <p className="text-slate-400 text-xs mb-1">Next Step:</p>
           <p className="text-cyan-400 text-sm font-bold font-['Orbitron'] tracking-wider uppercase">Choose Your Adventure</p>
        </div>
      </div>

      <div className="flex-1 w-full space-y-8">
        <div className="text-center md:text-left">
           <p className="text-slate-300 text-sm">Customize your character below. This avatar will represent you in all Majesty Math games.</p>
        </div>

        {/* Color Picker */}
        <div>
          <label className="text-xs font-bold uppercase text-slate-400 mb-3 block tracking-wider">Suit Color</label>
        ...
        {/* Helmet Picker */}
        <div>
          <label className="text-xs font-bold uppercase text-slate-400 mb-3 block tracking-wider">Helmet Style</label>
        ...
        {/* Badge Picker */}
        <div>
          <label className="text-xs font-bold uppercase text-slate-400 mb-3 block tracking-wider">Badge</label>
          <div className="flex gap-3 justify-center md:justify-start">
            {badges.map(b => (
              <button
                key={b}
